...
const NewColumnModal = ({ data, handleSetColumnData, isShown, setIsShown }) => {
	...
	const [showHelpText, setShowHelpText] = useState(data.invalidHelpText ? true : false);
	...
	return (
		<Dialog
			...
			confirmLabel='Apply'>
			<Pane>
				<Text color='gray600'>Column Header *</Text>
				{isEditingHeading ? (
			...
			<Pane display='flex' flexDirection='row' justifyContent='space-between' width='100%'>
				<Pane flex={1} display='flex' flexDirection='column' paddingRight={20}>
					...
					<SelectMenu
						hasFilter={false}
						hasTitle={false}
						options={Object.keys(options).map((option) => ({ value: option, label: options[option].label }))}
				...
				<Pane flex={1} display='flex' flexDirection='row' justifyContent='space-between'>
					<Pane display='flex' flexDirection='column' width='100%'>
						...
						<Pane display='flex' flexDirection='row' justifyContent='space-around'>
							<Pane display='flex' flexDirection='row' alignItems='center'>
								<Checkbox
									checked={newData.invalidAction ? (newData.invalidAction === 'reject' ? true : false) : false}
									onChange={(e) => {
										setNewData({ ...newData, invalidAction: e.target.checked ? 'reject' : 'warn' });
									}}
									size={16}
									name='invalidAction'
									label='Reject'
							...
							<Pane display='flex' flexDirection='row' alignItems='center'>
								<Checkbox
									checked={newData.invalidAction ? (newData.invalidAction === 'warn' ? true : false) : false}
									onChange={(e) => {
										setNewData({ ...newData, invalidAction: e.target.checked ? 'warn' : 'reject' });
									}}
									size={16}
									name='invalidAction'
									label='Warn'
			...
			{options[newData.type].ruleOptional ? (
				...
				<Pane display='flex' flexDirection='column' flex={3}>
					<Pane display='flex' flexDirection='row' marginTop={20} marginBottom={10}>
						<Text color='gray600'>Rule (optional)</Text>
						{isRuleTicked && <Switch marginLeft={10} checked={isRuleTicked} onChange={(e) => setIsRuleTicked(e.target.checked)} />}
					</Pane>
					{!isRuleTicked ? (
						<Switch marginLeft={10} checked={isRuleTicked} onChange={(e) => setIsRuleTicked(e.target.checked)} />
					) : (
						<ColumnRules type={newData.type} rule={newData.rule} handleUpdateRule={handleUpdateRule} />
					)}
				</Pane>
			) : (
				// Required Rules
			...
			<Pane display='flex' flexDirection='row' justifyContent='space-between' width='100%'>
				<Pane display='flex' flexDirection='column' flex={1} paddingRight={20}>
					<Pane display='flex' flexDirection='row' marginTop={20} marginBottom={10}>
						...
						{showHelpText && <Switch marginLeft={10} checked={showHelpText} onChange={(e) => setShowHelpText(e.target.checked)} />}
					</Pane>
					<Pane display='flex' flexDirection='row' justifyContent='space-between' width='100%' alignItems='center'>
						{!showHelpText ? (
							<Switch
								marginLeft={10}
